#-------------------------------------------------------------------------------------#
#--- automate counting of number of accidents involving cycles over Cambridgeshire ---#
#-------------------------------------------------------------------------------------#

#filter data to process only accidents involving at least one pedal cycle
cycles <- collisions %>%
  filter(Vehicle_1_Type == "Pedal cycle" | Vehicle_2_Type == "Pedal cycle" | Vehicle_3_Type =="Pedal cycle")

#determine grid boundaries
min_east <- min(cycles$Easting)
max_east <- max(cycles$Easting)
min_north <- min(cycles$Northing)
max_north <- max(cycles$Northing)
...
write_csv(number_collisions, "number_collisions_cycles.csv")